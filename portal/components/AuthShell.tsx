  ...
  return (
    <main className="auth-shell">
      <header className="site-header">
        ...
        <div className="secure-label"><ShieldIcon />안전한 연결</div>
      </header>
      ...
      <div className="auth-layout">
        <section className="brand-stage" aria-labelledby="brand-heading">
          <div className="stage-copy">
            ...
            <h1 id="brand-heading">하나의 계정으로,<br /><span>모든 서비스를.</span></h1>
            <p>Infinite Studio의 서비스를 더 빠르고 안전하게 이용하세요. 한 번의 인증으로 연결이 이어집니다.</p>
          </div>
          <div className="identity-orbit" aria-hidden="true">
            <i /><i /><i />
            <div className="identity-core"><FingerprintIcon /></div>
            <div className="service-node node-one"><b>A</b><span>Aztra</span><em>✓</em></div>
            <div className="service-node node-two"><b>S</b><span>Status</span><em>✓</em></div>
          </div>
          <div className="trust-row"><ShieldIcon /> 암호화된 인증 · Infinite Studio Identity</div>
        </section>
        ...
        <section className={`auth-panel${wide ? ' auth-panel-wide' : ''}`} aria-label="SSO 인증">
          <div className="auth-card screen-enter">
            <div className="request-badge">
              <b>IS</b>
              <span>{requestLabel}<strong>{requestName}</strong></span>
            </div>
            {children}
          ...
          <footer className="panel-footer">
            <a href="https://inft.kr/privacy">개인정보 처리방침</a><span>·</span>
            <a href="mailto:support@inft.kr">도움말</a><span>·</span><span>한국어</span>
          </footer>
        </section>
      </div>
    </main>
